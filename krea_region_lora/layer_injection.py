    ...
    def _make_hook(self, patches: list[LayerLoRAPatch]):
        def hook(_module, inputs, output):
            ...
            for patch in patches:
                ...
                delta = delta * float(patch.scale) * float(patch.regional.lora_strength) * float(patch.regional.delta_strength)
                out = out + (delta * mask.to(device=delta.device, dtype=delta.dtype)).to(dtype=out.dtype)
            return out
    ...
    *,
    ...
    for regional_index, regional in enumerate(stack.enabled_regions, start=1):
        ...
        for target, patch_data in loaded.items():
            ...
            matched += 1
        report.add(f"[{regional_index}] {regional.lora_name}: loaded_targets={len(loaded)} matched_layers={matched} skipped={skipped}")
...
def _sequence_mask_for_region(
    ...
    *,
    ...
    if seq_len == img_len:
        ...
        full = torch.full((int(x.shape[0]), seq_len, 1), float(text_token_strength), device=x.device, dtype=x.dtype)
        full[:, seq_len - img_len :, :] = token_mask
    else:
        resized = F.interpolate(token_mask.transpose(1, 2), size=seq_len, mode="linear", align_corners=False).transpose(1, 2)
