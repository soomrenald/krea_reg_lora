...
MLP_TARGETS = ("mlp", "ffn", "feed_forward", "feedforward", "fc1", "fc2", "up_proj", "down_proj", "gate_proj")
TEXT_ENCODER_HINTS = (
    "clip",
    "text_encoder",
    "text.encoder",
...
def is_krea_attention_lora_key(key: str) -> bool:
    ...
    if not any(part == "attn" or "attention" in part for part in parts):
        return False
    return any(target in parts or lowered.endswith(f".{target}.weight") or f".{target}." in lowered for target in ATTENTION_TARGETS)
...
def is_krea_attention_out_lora_key(key: str) -> bool:
    ...
    if any(target in parts for target in ("wq", "wk", "wv", "q_proj", "k_proj", "v_proj")):
        return False
    return any(target in parts or lowered.endswith(f".{target}.weight") or f".{target}." in lowered for target in ATTENTION_OUT_TARGETS)
...
def make_lora_branch_model(
    ...
    *,
    ...
    branch, _clip = comfy.sd.load_lora_for_models(base_model, None, lora, strength_model, 0.0, lora_metadata=metadata)
    return branch
