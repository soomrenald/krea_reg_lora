...
3. Regional prompt tokens must remain first-class tokens/ranges. Do not permanently collapse them into a pooled vector for the default implementation.
   ...
   Regional prompt tokens must be inserted/represented in the same runtime text token sequence that participates in Krea2 self-attention, or mapped to explicit token ranges in an equivalent conditioning sequence. Do not claim regional prompt-token attention bias exists unless `P_r` corresponds to real key positions in the attention tensor.
...
If real regional prompt token key positions cannot be identified, implementation must fail with a clear report. Do not create fake token ranges from pooled embeddings.
...
2. Log:
   ...
   - active attention backend/path for each hooked call, including whether it is torch SDPA, xformers, sage attention, Comfy optimized attention, or a custom fallback

   If the active backend cannot accept additive attention bias, stop and report the blocker instead of silently falling back to pooled-delta or CFG.
